...
module Language.Coq.Util.Containers
  ...
  -- ** Connected components
    connectedComponents
  , stronglyConnCompNE
  , connectedComponentsNE
  , stronglyConnComp'
  , connectedComponents'
  ,
  -- ** Topological sort
    stableTopoSortBy
  , stableTopoSortByPlus
  ,
...
setMapMaybeM
  :: (Applicative m, Ord b) => (a -> m (Maybe b)) -> Set a -> m (Set b)
setMapMaybeM f s = S.fromList . catMaybes <$> traverse f (S.toList s)

invertMap :: (Ord k, Ord v) => Map k (Set v) -> Map v (Set k)
invertMap m = M.unionsWith
  S.union
  [ M.fromSet (const $ S.singleton k) vs | (k, vs) <- M.toList m ]

connectedComponents :: Ord key => [(node, key, [key])] -> [SCC node]
connectedComponents graph =
  let edgeMap = M.unionWith S.union <*> invertMap $ M.fromList
        [ (k, S.fromList vs) | (_, k, vs) <- graph ]
...
nonEmpty_SCC (CyclicSCC []) =
  error "[internal] nonEmpty_SCC: empty connected component!"
...
simple_sccs
  ...
  -> [(vertex, [vertex])]
  -> [NonEmpty vertex]
simple_sccs makeCCs graph = map nonEmpty_SCC
  $ makeCCs [ (v, v, es) | (v, es) <- graph ] where
...
stableTopoSortBy
  :: (Foldable f1, Foldable f2, Foldable f3, Ord k)
  => (a -> f1 k) -- ^ The names of each object
  -> (a -> f2 k) -- ^ The dependencies of each object (they will /precede/ it)
  -> f3 a        -- ^ The objects to sort
  -> [a]
stableTopoSortBy names dependencies objs = (`appEndo` []) . snd $ execRWS
  (traverse_ visit $ M.keysSet objsById)
  ()
  IS.empty
 where
...
stableTopoSortByPlus
  :: (Foldable f1, Foldable f2, Foldable f3, Foldable f4, Ord k)
  => (a -> f1 k)
  -> (a -> f2 k)
  -> (k -> f3 k)
  -> f4 a
...
reachableFrom refl adjacencies v0 = execState (go v0) initial where
  go v = for_ (M.findWithDefault mempty v adjacencies)
    $ \v' -> unlessM (gets (v' `elem`)) $ modify (S.insert v') *> go v'
...
transitiveClosure refl adjacencies =
  M.fromSet (reachableFrom refl adjacencies)
    $  M.keysSet adjacencies
    <> fold adjacencies

transitiveClosureBy
  :: (Foldable f, Ord a) => Reflexivity -> (a -> Set a) -> f a -> Map a (Set a)
