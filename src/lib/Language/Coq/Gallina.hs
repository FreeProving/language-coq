...
--   However:
--
--   1. The @in@ annotation will only parse if the @/return_type/@ is present;
--      and
...
data Term
  ...
  | Cofix FixBodies
    -- ^ @cofix /cofix_bodies/@
  | Let Qualid [Binder] (Maybe Term) Term Term
    -- ^ @let /ident/ [/binders/] [: /term/] := /term/ in /term/@
  | LetTuple [Name] (Maybe DepRetType) Term Term
    -- ^ @let ( [/name/ , … , /name/] ) [/dep_ret_type/] := /term/ in /term/@
  | LetTick Pattern Term Term
    -- ^ @let ' /pattern/ := /term/ in /term/@
  | If IfStyle Term (Maybe DepRetType) Term Term
  ...
  | Arrow Term Term
    -- ^ @/term/ -> /term/@
  | App Term (NonEmpty Arg)
    -- ^ @/term/ /arg/ … /arg/@
  | ExplicitApp Qualid [Term]
    -- ^ @\@ /qualid/ [/term/ … /term/]@
  | InScope Term Ident
    -- ^ @/term/ % /ident/@
  | Match (NonEmpty MatchItem) (Maybe ReturnType) [Equation]
    -- ^ @
  ...
  | Qualid Qualid
    -- ^ @/qualid/@
  | RawQualid Qualid
    -- ^ @/qualid/@ like Qualid, but never printed with notation
  | Sort Sort
    -- ^ @/sort/@
  | Num Num
    -- ^ @/num/@
  ...
  | HsString Text
    -- ^ @& /string/@ – extra (for Haskell string literals)
  | HsChar Char
    -- ^ @&# /string/@ – extra (for Haskell character literals; /string/ is
...
data Binder
  = Inferred Explicitness Name
    -- ^ @/name/@ or @{ /name/ }@
  | Typed Generalizability Explicitness (NonEmpty Name) Term
    -- ^ @/generalizability/@ @( /name/ … /name/ : /term/ )@
    --   or @/generalizability/@ @{ /name/ … /name/ : /term/ }@
...
-- | @/pattern/ ::=@
data Pattern
  = ArgsPat Qualid [Pattern]
    -- ^ @/qualid/ /pattern/ … /pattern/@ – should start with a constructor
  | ExplicitArgsPat Qualid (NonEmpty Pattern)
    -- ^ @\@ /qualid/ /pattern/ … /pattern/@
  | InfixPat Pattern Op Pattern
    -- ^ @/pattern/ /op/ /pattern/@ – extra
  | AsPat Pattern Qualid
    -- ^ @/pattern/ as /ident/@
  | InScopePat Pattern Ident
    -- ^ @/pattern/ % /ident/@
  | QualidPat Qualid
    -- ^ @/qualid/@ – should be only variables
  | UnderscorePat
    -- ^ @_@
  | NumPat Num
    -- ^ @/num/@
...
-- | @/definition/ ::=@
data Definition
  = DefinitionDef Locality Qualid [Binder] (Maybe Term) Term
...
data Inductive
  = Inductive (NonEmpty IndBody) [NotationBinding]
    ...
    --   @
  | CoInductive (NonEmpty IndBody) [NotationBinding]
    -- ^ @
...
data Fixpoint
  = Fixpoint (NonEmpty FixBody) [NotationBinding]
    ...
    --     [where /notation_binding/ and … and /notation_binding/] .
    --   @
  | CoFixpoint (NonEmpty FixBody) [NotationBinding]
    -- ^ @
...
--   incomplete
data ModuleSentence
  = ModuleImport ImportExport (NonEmpty ModuleIdent)
    -- ^ @/import_export/ /qualid/ … /qualid/ .@
  | Require (Maybe ModuleIdent) (Maybe ImportExport) (NonEmpty ModuleIdent)
...
data InstanceDefinition
  = InstanceDefinition Qualid [Binder] Term [(Qualid, Term)] (Maybe Proof)
    -- ^ @
    --   Instance /ident/ [/binders/] : /term/ :=
    --     { [/ident/ := /term/ ; … ; /ident/ := /term/] } [/proof/] .
    --   @
